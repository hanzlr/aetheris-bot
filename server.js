...
app.post('/givebox', authMiddleware, async (req, res) => {
  try {
    ...
    await supabase.from('lootboxes').insert({ user_id: userId, box_type: boxType })
    const guild = req.app.get('guild')
...
app.post('/resetuser', authMiddleware, async (req, res) => {
  try {
    ...
    await supabase.from('levels').update({ xp: 0, level: 0, total_messages: 0, coins: 0, bank: 0, last_daily: null }).eq('user_id', userId)
...
app.post('/event/start', authMiddleware, async (req, res) => {
  try {
    ...
    if (eventType === 'loot_rain') {
      const { data: allMembers } = await supabase.from('levels').select('user_id')
      for (const member of allMembers || []) {
        await supabase.from('lootboxes').insert({ user_id: member.user_id, box_type: 'common' })
      }
    }
...
app.post('/event/stop', authMiddleware, async (req, res) => {
  try {
    ...
    if (channelId) {
      ...
      const channel = guild.channels.cache.get(channelId)
      if (channel) {
        await channel.send(`⚠️ **EVENT TELAH BERAKHIR!**\n\n— *Admin UMB Esport*`)
      }
    }
