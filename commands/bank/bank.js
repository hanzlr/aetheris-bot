...
export async function handleBank(interaction) {
  ...
  if (jumlah <= 0) return interaction.reply({ content: '❌ Jumlah harus lebih dari 0!', flags: 64 })

  if (sub === 'deposit') {
    if ((data.coins || 0) < jumlah) return interaction.reply({ content: `❌ Koin wallet tidak cukup! Kamu punya **${data.coins || 0} koin**.`, flags: 64 })
  ...
  if (sub === 'withdraw') {
    if ((data.bank || 0) < jumlah) return interaction.reply({ content: `❌ Saldo bank tidak cukup! Kamu punya **${data.bank || 0} koin** di bank.`, flags: 64 })
