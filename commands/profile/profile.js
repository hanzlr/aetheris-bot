...
export const BADGES = {
  ...
  daily_streak: { emoji: '📅', name: 'Consistent', desc: 'Claim daily 7 hari berturut' },
}
...
export async function checkBadges(userId, userData) {
  ...
  if (!owned.includes('chatterbox') && userData.total_messages >= 100) newBadges.push('chatterbox')
  if (!owned.includes('rising_star') && userData.level >= 5) newBadges.push('rising_star')
  if (!owned.includes('legend') && userData.level >= 20) newBadges.push('legend')
  if (!owned.includes('rich') && (userData.coins || 0) >= 10000) newBadges.push('rich')
  if (!owned.includes('saver') && (userData.bank || 0) >= 5000) newBadges.push('saver')
...
export const profileData = new SlashCommandBuilder()
  ...
  .addSubcommand(sub =>
    sub.setName('badges')
      .setDescription('Lihat semua badge yang kamu punya')
  )
...
export async function handleProfile(interaction) {
  ...
  if (sub === 'view') {
    ...
    if (!data) return interaction.reply({ content: '❌ User ini belum punya akun!', flags: 64 })
    ...
    const badgeEmojis = userBadges?.map(b => BADGES[b.badge_id]?.emoji || '🏅').join(' ') || 'Belum ada badge'
    ...
    if (newBadges.length > 0) {
      const newBadgeText = newBadges.map(b => `${BADGES[b].emoji} ${BADGES[b].name}`).join(', ')
  ...
  if (sub === 'badges') {
    ...
    if (!userBadges || userBadges.length === 0) {
      return interaction.reply({ content: '❌ Kamu belum punya badge! Terus aktif di server.', flags: 64 })
    }
