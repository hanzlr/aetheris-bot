...
export const levelData = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) => sub.setName("rank").setDescription("Lihat rank kamu"))
  .addSubcommand((sub) =>
    sub.setName("leaderboard").setDescription("Lihat leaderboard server"),
  );
...
export async function handleLevel(interaction) {
  ...
  if (sub === "leaderboard") {
    ...
    if (!data || data.length === 0)
      return interaction.reply({ content: "❌ Belum ada data!", flags: 64 });
