...
export const gameData = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      .setName("crash")
      .setDescription("🚀 Game Crash — Cashout sebelum crash! (Premium only)")
      .addIntegerOption((option) =>
        option
          .setName("taruhan")
          .setDescription("Jumlah koin yang mau ditaruhkan")
...
export async function handleGame(interaction) {
  ...
  if (sub === "crash") await handleCrash(interaction);
}
...
async function handleDice(interaction) {
  ...
  if (taruhan <= 0)
    return interaction.reply({
      content: "❌ Taruhan harus lebih dari 0!",
      flags: 64,
    });
  ...
  if ((data.coins || 0) < taruhan)
    return interaction.reply({
      content: `❌ Koin tidak cukup! Kamu punya **${data.coins || 0} koin**.`,
      flags: 64,
    });
...
async function handleCoinflip(interaction) {
  ...
  const newCoins = menang
    ? (data.coins || 0) + taruhan
    : (data.coins || 0) - taruhan;
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: "✋ Pilihan Kamu",
        value: pilihan === "heads" ? "👑 Heads" : "🔵 Tails",
        inline: true,
      },
      {
        name: "🪙 Hasil",
        value: hasil === "heads" ? "👑 Heads" : "🔵 Tails",
        inline: true,
      },
      {
        name: "📊 Hasil",
...
async function handleBlackjack(interaction) {
  ...
  if (activeGames.has(user.id)) {
    return interaction.reply({
      content: "❌ Kamu masih punya game blackjack yang belum selesai!",
      flags: 64,
    });
  }
...
export async function handleBlackjackButton(interaction) {
  ...
  if (!game)
    return interaction.reply({
      content: "❌ Tidak ada game blackjack aktif!",
      flags: 64,
    });
  ...
  if (interaction.customId === "bj_hit") {
    ...
    if (playerTotal > 21) {
      ...
      embed.addFields(
        {
          name: "📊 Hasil",
          value: `😢 Bust! Kamu melebihi 21! -${taruhan} koin`,
        },
