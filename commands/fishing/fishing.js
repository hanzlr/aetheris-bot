...
function getRandomFish(pancingId = "pancing_bambu", umpanId = "cacing") {
  ...
  if (roll < (cumulative += chances.common))
    return getFromPool(["lele", "mujair", "nila"]);
  if (roll < (cumulative += chances.uncommon))
    return getFromPool(["kakap", "kerapu", "bandeng"]);
  if (roll < (cumulative += chances.rare))
    return getFromPool(["hiu", "pari", "tuna"]);
  if (roll < (cumulative += chances.epic))
    return getFromPool(["coelacanth", "arapaima"]);
...
export const fishingData = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) => sub.setName("cast").setDescription("Mancing ikan!"))
  .addSubcommand((sub) =>
    sub
      .setName("sell")
...
export async function handleFishing(interaction) {
  ...
  if (sub === "cast") {
    ...
    // Cek event fishing frenzy
    const { data: eventData } = await supabase
      .from("events")
      .select("*")
      .eq("is_active", true)
      .eq("event_type", "fishing_frenzy")
    ...
    if (diff < cooldown) {
      ...
      return interaction.reply({
        content: `⏳ Tunggu **${sisa} detik** lagi sebelum mancing!${eventData ? " 🎣 Fishing Frenzy aktif!" : ""}`,
        flags: 64,
      });
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "💰 Estimasi Harga",
          value: `${fish.minPrice}-${fish.maxPrice} koin`,
          inline: true,
        },
        {
          name: "🎣 Pancing",
          value: `${pancing.emoji} ${pancing.name}`,
          inline: true,
        },
        {
          name: "🪱 Umpan",
          value: `${umpan.emoji} ${umpan.name}`,
          inline: true,
        },
      )
      .setColor(color)
  ...
  if (sub === "sell") {
    ...
    if (fishChoice) {
      ...
      if (!fishData || fishData.quantity === 0) {
        return interaction.reply({
          content: `❌ Kamu tidak punya **${FISH_LIST[fishChoice].name}**!`,
          flags: 64,
        });
      }
      ...
      const pricePerFish =
        Math.floor(Math.random() * (fish.maxPrice - fish.minPrice + 1)) +
        fish.minPrice;
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "🐟 Ikan",
            value: `${fish.emoji} ${fish.name} x${fishData.quantity}`,
            inline: true,
          },
          {
            name: "💰 Harga Per Ekor",
            value: `${pricePerFish} koin`,
            inline: true,
          },
          {
            name: "💳 Total Didapat",
            value: `+${totalPrice} koin`,
            inline: true,
          },
          {
            name: "💳 Koin Sekarang",
            value: `${(userData.coins || 0) + totalPrice} koin`,
            inline: false,
          },
        )
        .setColor(0x57f287)
      ...
      if (!allFish || allFish.length === 0) {
        return interaction.reply({
          content: "❌ Kamu tidak punya ikan! Mancing dulu dengan /fish cast.",
          flags: 64,
        });
      }
      ...
      for (const fishItem of allFish) {
        ...
        totalCoins += total;
        soldList.push(
          `${fish.emoji} ${fish.name} x${fishItem.quantity} = ${total} koin`,
        );
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "💳 Total Didapat",
            value: `+${totalCoins} koin`,
            inline: true,
          },
          {
            name: "💳 Koin Sekarang",
            value: `${(userData.coins || 0) + totalCoins} koin`,
            inline: true,
          },
        )
        .setColor(0x57f287)
